...
function agregarEvento() {
    ...
    let nuevoEvento = { 
        nombre: nombreEvento, 
        descripciones: [descripcionEvento], // Cambiamos a un array
        fecha: fechaEvento
    };
...
function agregarTarea() {
    ...
    let nuevaTarea = { 
        nombre: nombreTarea, 
        descripciones: [descripcionTarea], // Cambiamos a un array
        fecha: fechaTarea 
    };
...
function mostrarEventos() {
    ...
    eventos.forEach((evento, index) => {
        ...
        elementoLista.innerHTML = `<strong>${evento.nombre}</strong><br>
                                   <span id="descripcionEvento${index}">${evento.descripciones}</span>
                                   <input type="text" id="inputDescripcionEventoActualiza${index}" placeholder="Nueva Descripción">
                                   ...
                                   <button class="eliminarDescripcion" style="display: ${evento.descripciones ? 'inline' : 'none'};">Eliminar Descripción</button><br>
        ...
        botonEliminarDescripcion.addEventListener('click', () => {
            evento.descripciones.pop(); // Eliminar la última descripción
            localStorage.setItem("eventos", JSON.stringify(eventos));
...
function mostrarTareas() {
    ...
    tareas.forEach((tarea, index) => {
        ...
        elementoTarea.innerHTML = `<strong>${tarea.nombre}</strong><br>
                                  <span id="descripcionTarea${index}">${tarea.descripciones}</span>
                                    <input type="text" id="inputDescripcionActualiza${index}" placeholder="Nueva Descripción">
                                  ...
                                  <button class="eliminarDescripcion" style="display: ${tarea.descripciones ? 'inline' : 'none'};">Eliminar Descripción</button><br>
        ...
        botonEliminarDescripcionTarea.addEventListener('click', () => {
            tarea.descripciones.pop(); // Eliminar la última descripción
            localStorage.setItem("tareas", JSON.stringify(tareas));
...
function agregarDescripcionEvento(index) {
    ...
    if (nuevaDescripcion) {
        eventos[index].descripciones.push(nuevaDescripcion); // Agregar nueva descripción
        localStorage.setItem("eventos", JSON.stringify(eventos));
        mostrarEventos(); // Actualizar la vista
    }
}
...
function agregarDescripcionTarea(index) {
    ...
    if (nuevaDescripcion) {
        tareas[index].descripciones.push(nuevaDescripcion); // Agregar nueva descripción
        localStorage.setItem("tareas", JSON.stringify(tareas));
        mostrarTareas(); // Actualizar la vista
    }
}
...
function actualizarDescripcionEvento(index) {
    let nuevaDescripcion = document.getElementById(`inputDescripcionEventoActualiza${index}`).value.trim();
    if (nuevaDescripcion) {
...
function actualizarDescripcionTarea(index) {
    let nuevaDescripcion = document.getElementById(`inputDescripcionTareaActualiza${index}`).value.trim();
    if (nuevaDescripcion) {
