...
function agregarTarea(eventoIndex) {
  ...
  if (tareaInput) {
    ...
    eventos[eventoIndex].tareas.push({
      nombre: tareaInput,
      completa: false,
      color: 'verde' // Color predeterminado al agregar una tarea nueva
    });
...
function renderizarEventos() {
  ...
  eventos.forEach((evento, eventoIndex) => {
    ...
    evento.tareas.forEach((tarea, tareaIndex) => {
      ...
      // Agregar una clase CSS según el color seleccionado para la tarea (rojo, naranja, verde)
      switch (tarea.color) {
        case 'red':
          tareaElement.classList.add('red');
          break;
        case 'orange':
          tareaElement.classList.add('orange');
          break;
        case 'green':
          tareaElement.classList.add('green');
          break;
        default:
          break;
      }
